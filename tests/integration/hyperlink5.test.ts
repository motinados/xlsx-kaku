...
describe("hyperlink with multiple sheets", () => {
  const testName = "hyperlink5";
  ...
  test("worksheet 1", () => {
    ...
    // It should be a problem-free difference.
    deletePropertyFromObject(
      expectedObj.worksheet.hyperlinks.hyperlink,
      "@_xr:uid"
    );
    deletePropertyFromObject(
      actualObj.worksheet.hyperlinks.hyperlink,
      "@_xr:uid"
    );
  ...
  test("worksheet 2", () => {
    ...
    const expectedXml = readFileSync(expectedXmlPath, "utf8");
    const actualXmlPath = resolve(actualFileDir, "xl/worksheets/sheet2.xml");
    const actualXml = readFileSync(actualXmlPath, "utf8");
